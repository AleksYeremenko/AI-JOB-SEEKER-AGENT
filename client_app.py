...
class AIJobSeekerApp(ctk.CTk):
    def __init__(self):
        ...
        # 1. Сделали стартовое окно больше
        self.geometry("700x850")
        # 2. Разрешаем растягивать окно, но запрещаем сжимать меньше 600x800
    ...
    def create_widgets(self):
        # --- Увеличенные шрифты для поддержки высоких разрешений ---
        ...
        self.title_label = ctk.CTkLabel(
            self.main_frame, text="Automate your job search",
            font=title_font, text_color=TEXT_MAIN
        )
        self.title_label.pack(anchor="w", pady=(0, 8))

        self.subtitle_label = ctk.CTkLabel(
            self.main_frame, text="Set up your AI agent in seconds.",
            font=subtitle_font, text_color=TEXT_MUTED
        )
        self.subtitle_label.pack(anchor="w", pady=(0, 40))

        # --- Поля ввода (увеличена высота до 50) ---
        self.position_entry = ctk.CTkEntry(
            self.main_frame, placeholder_text="Target Role (e.g. Python Backend)",
        ...
        self.stack_entry = ctk.CTkEntry(
            self.main_frame, placeholder_text="Tech Stack (e.g. Python, Docker, AWS)",
            height=50, fg_color=FRAME_COLOR, border_color=FRAME_COLOR,
            text_color=TEXT_MAIN, font=input_font, corner_radius=8
        )
        ...
        self.email_entry = ctk.CTkEntry(
            self.main_frame, placeholder_text="Contact Email",
            height=50, fg_color=FRAME_COLOR, border_color=FRAME_COLOR,
            text_color=TEXT_MAIN, font=input_font, corner_radius=8
        )
        ...
        self.city_combo = ctk.CTkComboBox(
            self.row_frame, values=["Warsaw", "Krakow", "Wroclaw", "Remote"],
            height=50, fg_color=FRAME_COLOR, border_color=FRAME_COLOR,
        ...
        self.job_type_combo = ctk.CTkComboBox(
            self.row_frame, values=["Backend", "Frontend", "Fullstack", "DevOps"],
            height=50, fg_color=FRAME_COLOR, border_color=FRAME_COLOR,
            button_color=FRAME_COLOR, button_hover_color=ACCENT_COLOR,
            dropdown_fg_color=FRAME_COLOR, font=input_font, corner_radius=8
        )
        ...
        self.slider_label = ctk.CTkLabel(
            self.main_frame, text="Minimum Stack Match: 50%",
            font=subtitle_font, text_color=TEXT_MUTED
        )
        self.slider_label.pack(anchor="w", pady=(20, 5))

        self.match_slider = ctk.CTkSlider(
            self.main_frame, from_=10, to=100, number_of_steps=90,
            button_color=ACCENT_COLOR, button_hover_color=TEXT_MAIN, progress_color=ACCENT_COLOR,
            command=self.update_slider_label, height=20
        )
        ...
        self.cv_label = ctk.CTkLabel(
            self.cv_frame, text="No resume uploaded",
            font=subtitle_font, text_color=TEXT_MUTED
        )
        self.cv_label.pack(pady=(15, 10))

        self.upload_btn = ctk.CTkButton(
            self.cv_frame, text="Upload CV (PDF)", command=self.upload_cv,
            height=40, width=180,
            fg_color="transparent", border_width=1.5, border_color=TEXT_MUTED,
            hover_color=FRAME_COLOR, text_color=TEXT_MAIN, font=ctk.CTkFont(size=14, weight="bold")
        )
        self.upload_btn.pack(pady=(0, 15))

        # --- Главная кнопка запуска (увеличена и прижата к низу) ---
        self.save_btn = ctk.CTkButton(
            self.main_frame, text="Launch Agent", command=self.save_settings,
            height=60, fg_color=ACCENT_COLOR, hover_color=ACCENT_HOVER,
            font=main_btn_font, corner_radius=8
        )
    ...
    def run_browser_agent(self, user_data):
        """Запускает невидимый браузер и взаимодействует с сервером"""
        ...
        try:
            with sync_playwright() as p:
                print("🌐 Запускаю невидимый браузер в фоне...")

                # Фоновый режим (headless=True) - браузер работает без окна
                browser = p.chromium.launch(headless=True)

                # Жестко фиксируем размер окна (FullHD), чтобы скрины всегда были одинаковыми
                ...
                # 1. Делаем скрытый скриншот
                screenshot_bytes = page.screenshot(full_page=False)
                img_b64 = base64.b64encode(screenshot_bytes).decode('utf-8')

                print("📤 Отправляю скриншот на сервер для анализа...")
                ...
                if response.status_code == 200:
                    ...
                    for elem in elements:
                        if elem.get("type") == "button_apply":
                            ...
                            # Сохраняем скриншот после клика для проверки
